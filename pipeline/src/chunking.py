...
def chunk_sentence_aware(
    ...
    absolute_max_tokens: int = 500, # Hard safety ceiling for e5-large (max 512)
    model_name: str = "intfloat/multilingual-e5-large",
) -> pd.DataFrame:
    """
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="sentence_aware chunking"):
        ...
        # Step 1: Expand oversized sentences and track token length of each sentence
        ...
        for sent in sentences:
            ...
            if tok_len <= max_tokens_per_sentence:
                ...
                # Token-level fallback: split long sentence into sub-chunks
        ...
        # Step 2: Merge sentences with strict total token constraint
        chunks_for_doc = []
        start = 0
        while start < len(expanded_with_lengths):
            ...
            while end < len(expanded_with_lengths):
                ...
                # Break if max sentence count reached
                if len(current_chunk) == max_sentences:
                    break
                
                # Break if adding this sentence would exceed the token limit
                # (Only break if the chunk already has at least 1 sentence)
            ...
            # Advance the start pointer, accounting for sentence overlap
            step = max(len(current_chunk) - overlap_sentences, 1)
            start += step

        # Step 3: Format and collect output records
        total = len(chunks_for_doc)
        for idx, text in enumerate(chunks_for_doc):
            all_chunks.append({
                "chunk_id": f"{doc_id}_c{idx:04d}",
