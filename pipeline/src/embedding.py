...
def build_faiss_index(npy_path: str, index_output_path: str, dim: int = 1024):
    ...
    # 1. Infer the number of vectors from the file size on disk (4 bytes per float32)
    file_size = os.path.getsize(npy_path)
    num_vectors = file_size // (4 * dim)
    
    # 2. Load file as raw memmap (read-only)
    ...
    # 3. Copy to RAM for in-place L2 normalization (memmap mode 'r' is read-only)
