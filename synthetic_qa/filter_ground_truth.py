...
def filter_qa_to_target():
    # 1. Định nghĩa đường dẫn hệ thống
    ...
    if not qa_path.exists():
        raise FileNotFoundError(f"Không tìm thấy tập dữ liệu gốc tại: {qa_path}")

    # 2. Đọc 863 câu QA hiện tại vào DataFrame
    qa_list = []
    ...
    print(f"[INFO] Tổng số câu QA đầu vào: {total_current_qa}")

    # 3. Tải metadata để lấy thông tin nhãn tầng (strata attributes)
    ...
    # 4. Trộn nhãn tầng vào tập dữ liệu QA
    df_merged = df_qa.merge(df_meta, on="doc_id", how="left")

    # 5. Tính toán phân bổ tỷ lệ thuận cho mục tiêu 150 câu
    total_target = 150
    ...
    # Bù trừ sai số phần dư do làm tròn sàn (Rounding adjustments)
    ...
    # 6. Trích xuất mẫu ngẫu nhiên theo hạt giống cố định (random_state=42)
    sampled_frames = []
    ...
    # 7. Ghi file kết quả chuẩn hóa định dạng Ragas
    ...
    print(f"[SUCCESS] Đã lọc thành công {len(df_output)} câu Ground Truth lưu tại: {output_path}")
